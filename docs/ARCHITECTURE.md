...
- D1 psychology_style_bindings owns per-administrator group pools and account overrides. Stable account hashing selects a pool member, frozen in psychologyAutomation.styleId at task creation. Shared Canvas modules render twenty text-only cover/content styles, loaded via blob modules in Browser Run and local static modules in Chrome.
