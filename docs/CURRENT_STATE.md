...
- The admin-only `/tiktok-video-detail` page filters bridge-authorized accounts and TikTok video IDs, then shows one video's private metrics, interactive retention and like timelines, traffic sources, audience breakdowns, and comments for TikTok Studio accuracy checks.
...
- Novel AI Operations reads up to 30 recent synchronized videos per selected account from the official hosted bridge, within the 10-day strategy window. The payload includes complete curves, completion/watch-time, reach, engagement, traffic-source, and audience-breakdown metrics. Bridge failures remain non-blocking and publishing continues to use the existing safety controls.
