...
- The online psychology sidebar now has three independent templates: the existing four-image test at `/psychology`, paper collage at `/psychology-collage`, and interactive test at `/psychology-target-2`. Psychology templates have been removed from the mid-video workbench; existing URLs, task types, settings and records are preserved.
