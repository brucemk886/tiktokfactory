...
每 5 分钟跑 `syncWorkerState`：库存设置仍走 `POST /api/worker/sync`（`redditMixSettings`、`retentionHours: 48`，首次还会上传小说库）。**本机工人 `windows-local` 已切 SQLite**：官方记录读写 `official-history.sqlite` 的 `publishing_*` 表，同步走 `POST /api/worker/publish-records/sync`（protocolVersion=2，按 outbox `seq` 分页确认）。旧 `/api/worker/sync` 不再带官方记录。GeeLark 仍只在 `publish-records.json`，不上传。开关文件是 `work/official-publish-store.json`。回滚与备份见 `docs/publish-records-sqlite-cutover.md`。
...
- 成片叠层：前几秒居中 Reddit 文案框（平台 logo + 平台名，右侧 `code：推广码`，标题用口播第一句）；框还在时不烧字幕，框消失后才出。官方通道另叠左上角推广码角标（`burnNovelBadge`，GeeLark 默认关）。
- 模板1 素材子文件夹（`generation.assetFolders`）：素材组仍是 `assetLibraryRoot` 下的一级目录。组里按真实目录树勾选：点名称展开子夹，勾父夹等于全选其下所有夹，也可以只勾某一层。列表只显示文件夹和条数，不列出夹里的片子、也不提供预览。全勾或不传 = 整组。混剪按路径前缀过滤（勾 `minecraft/town` 不会抽到 `minecraft` 根下的片子）。点开头 / 下划线夹（`.meowload`、`_visual-review`）既不当素材组也不进勾选。模板2 不走这套。
...
- 日配额：任务 `max(5000, 活跃账号 × 10)` 条；上传按图片计 `max(5000, 活跃账号 × 10 × 8)` 张（图文一帖最多 6 图 + 恢复重传余量），1000 号即任务 10000 条/日、上传 80000 张/日。
...
| 中台日配额 | 任务 max(5000, 号数×10)；上传另按图片计 ×8 | 够（图文 3000 帖 × 6 图 = 1.8 万张 < 8 万） |
...
- 中台任务日配额 `max(5000, 账号×10)`，1000 号即 10000 帖；上传已按图片计（×8），图文不再先撞上传墙。超过 10 帖/号/日要另议。
