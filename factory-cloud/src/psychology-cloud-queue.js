...
export async function runCloudPhoto(env, job, deps={}) {
  ...
  if(pending.length) {
    ...
    let images;
    try {
      images=await renderer.renderBatch(pending.map(({source,index})=>({source,index,styleId:payload.psychologyAutomation.styleId||'',styleDefinition:payload.psychologyAutomation.styleDefinition||null,template:payload.psychologyAutomation.template,imageData:backgrounds.get(index)||''})));
