...
test("signed Gemini video source streams the private R2 object", async () => {
  ...
  assert.equal(new URL(signedUrl).pathname, "/api/integrations/gemini-video-source/analysis-1/source.mp4");
  const request = new Request(signedUrl);
  const response = await handleGeminiVideoSource(request, env, new URL(signedUrl), now);
  assert.equal(response.status, 200);
  const legacyUrl = signedUrl.replace("/source.mp4?", "?");
  assert.equal((await handleGeminiVideoSource(new Request(legacyUrl), env, new URL(legacyUrl), now)).status, 200);
