...
export async function loadCardModules(env) {
  return Promise.all(['psychology-text-card.js', 'psychology-card-renderer.js', 'psychology-card-runtime.js','psychology-visual-styles.js','psychology-styled-card.js'].map(async name => {
...
export async function openCloudCardRenderer(env, sources, driver, sleep = ms => new Promise(done => setTimeout(done, ms))) {
  ...
  try {
    ...
    await page.evaluate(async ([cardsSource, rendererSource, runtimeSource, stylesSource, styledSource]) => {
      const cardsUrl = URL.createObjectURL(new Blob([cardsSource], {type:'text/javascript'}));
      const stylesUrl=URL.createObjectURL(new Blob([stylesSource],{type:'text/javascript'}));
      const styledUrl=URL.createObjectURL(new Blob([styledSource.replace('"./psychology-text-card.js"',JSON.stringify(cardsUrl)).replace('"./psychology-visual-styles.js"',JSON.stringify(stylesUrl))],{type:'text/javascript'}));
      const rendererUrl = URL.createObjectURL(new Blob([
        rendererSource.replace('"./psychology-text-card.js"', JSON.stringify(cardsUrl)).replace('"./psychology-styled-card.js"',JSON.stringify(styledUrl))
      ...
      URL.revokeObjectURL(stylesUrl);URL.revokeObjectURL(styledUrl);URL.revokeObjectURL(cardsUrl); URL.revokeObjectURL(rendererUrl);
    }, sources);
    return {
