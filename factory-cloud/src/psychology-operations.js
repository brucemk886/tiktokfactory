...
export async function handlePsychologyOperations(request, env, url, session) {
  ...
  try {
    ...
    const store=await measured("groups",()=>loadGroupStore(env.DB)), project=findProjectForModule(store,"psychology"), allowed=userAllowedGroupIds(user);
    ...
    const archived=accountsFromLatestArchive(await measured("directory",()=>listLatestArchiveAccounts(env.DB)));
    ...
    const [videosByAccount,recordRows,itemRows,history,autopilotRows]=await Promise.all([
      measured("videos",()=>loadVideosForAccounts(env,env.DB,accounts.map(a=>a.schema),100,{concurrency:24,repair:false})),
      measured("receipts",()=>env.DB.prepare("SELECT value_json FROM factory_publish_records WHERE created_at>=? AND created_at<? ORDER BY created_at DESC LIMIT 10001").bind(window.previousStart,Date.now()+1).all()),
      measured("items",()=>env.DB.prepare(`SELECT i.id,i.source_id,i.batch_id,i.job_id,i.connection_id,i.receipt_json,i.publish_group_id,json_object('name',json_extract(b.config_json,'$.name'),'mediaType',json_extract(b.config_json,'$.mediaType'),'template',json_extract(b.config_json,'$.template')) config_json,b.created_at,
      ...
      detailed ? Promise.resolve(null) : measured("history",()=>loadResolvedItems(env.DB,window.previousStart-HISTORY_MS)),
      detailed ? Promise.resolve({results:[]}) : measured("pilots",()=>env.DB.prepare(`SELECT i.id,i.batch_id,i.connection_id,i.schedule_at,i.deleted_at,i.receipt_json,i.execution_status,
    ...
    const {framework}=await measured("framework",()=>frameworkFor(env,{accounts,window,media,videosByAccount,records,history,matureOnly:false}));
