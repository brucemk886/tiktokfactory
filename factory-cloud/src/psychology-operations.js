...
export async function frameworkFor(env,{accounts,window,media="photo",videosByAccount,records,history}){
...
export async function handlePsychologyOperations(request, env, url, session) {
  ...
  try {
    ...
    const [videosByAccount,recordRows,itemRows,history]=await Promise.all([
    ...
    const {framework}=await frameworkFor(env,{accounts,window,media,videosByAccount,records,history});
    return json({...report,content:null,framework,evolution:EVOLUTION,groups,projectName:project?.name||"心理学",updatedAt:Date.now(),
