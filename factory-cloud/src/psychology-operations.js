import { loadReportContext,readReportQueries } from './psychology-report-data.js';
...
import { scopedAnalyticsAccounts } from "./official.js";
...
import { loadVideosForAccounts, loadReportVideosForAccounts, reportVideoCacheQuery } from "./official-archive-store.js";
...
import { loadResolvedItems, resolvedItemsQuery, resolveItems, EVOLUTION } from "./psychology-copy-evolution.js";
...
export async function handlePsychologyOperations(request, env, url, session) {
  ...
  const measured=async(name,fn)=>{const at=Date.now();try{return await fn();}finally{timings[name]=Date.now()-at;}};
  const respond=data=>{const at=Date.now(),response=json(data);timings.serialize=Date.now()-at;timings.total=Date.now()-started;
    response.headers.set('server-timing',Object.entries(timings).map(([key,value])=>key+';dur='+value).join(', '));return response;};
  try {
    ...
    const {store,archived}=await measured("scope",()=>loadReportContext(env.DB));
    const project=findProjectForModule(store,"psychology"), allowed=userAllowedGroupIds(user);
    ...
    const inputs=await measured("queries",()=>readReportQueries(env.DB,{
      receipts:env.DB.prepare("SELECT value_json FROM factory_publish_records WHERE created_at>=? AND created_at<? ORDER BY created_at DESC LIMIT 10001").bind(window.previousStart,Date.now()+1),
      items:env.DB.prepare(`SELECT i.id,i.source_id,i.batch_id,i.job_id,i.connection_id,i.receipt_json,i.publish_group_id,json_object('name',json_extract(b.config_json,'$.name'),'mediaType',json_extract(b.config_json,'$.mediaType'),'template',json_extract(b.config_json,'$.template')) config_json,b.created_at,
        ...
        ORDER BY b.created_at DESC,i.id LIMIT 5001`).bind(window.start-SCHEDULE_LEAD_MS,window.end),
      history:detailed?null:resolvedItemsQuery(env.DB,window.previousStart-HISTORY_MS),
      pilots:detailed?null:env.DB.prepare(`SELECT i.id,i.batch_id,i.connection_id,i.schedule_at,i.deleted_at,i.receipt_json,i.execution_status,
        ...
        ORDER BY i.schedule_at,i.id LIMIT 20001`).bind(JSON.stringify(groupIds),accountIds,window.start-DAY,window.end,window.start/1000,window.end/1000),
      videos:reportVideoCacheQuery(env.DB,accounts.filter(a=>a.latestSyncAt>0).map(a=>a.schema)),
      topics:!detailed&&media==='photo'?env.DB.prepare("SELECT video_url,topics_json FROM psychology_peer_hits WHERE media_type='photo'"):null,
    }));
    const {receipts:recordRows,items:itemRows,pilots:autopilotRows={results:[]}}=inputs;
    const history=detailed?null:resolveItems(inputs.history.results);
    const videosByAccount=await measured('videos',()=>loadReportVideosForAccounts(env,env.DB,accounts,inputs.videos.results));
    ...
    if(detailed){
      ...
      return respond({content:{...detail,rows:detail.rows.filter(inWindow)},updatedAt:Date.now()});
    }
    const report=await measured("execution",()=>buildOperationsReport({window,accounts,videosByAccount,records,items,media}));
    const {framework}=await measured("framework",()=>frameworkFor(env,{accounts,window,media,videosByAccount,records,history,matureOnly:false,peerRows:inputs.topics?.results}));
    ...
    return respond({...report,autopilot,content:null,framework,evolution:EVOLUTION,groups,projectName:project?.name||"心理学",updatedAt:Date.now(),
