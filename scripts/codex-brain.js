...
function narratorVoiceRules(input) {
  return input.narratorGender === "female"
    ? `叙述声音：第一人称按成年女性口吻写（I / my）。不要改成男主自述，也不要把叙述者换成相反性别。后面按女声配音。`
...
function openingVariantHardRules(input, { lockStyles = false } = {}) {
  ...
  const sourceNoun = isPeerTranscriptSource(input) ? "对照口播" : "原文";
...
5. script 的前三句必须严格执行“事实炸点 → 错误预期或后果 → 反转信息缺口”，并且和 openingTitle 对准同一冲突；第四句起到 CTA 之前只换说法；全文是连续口播，大约 200 到 280 个英文单词，最多 320 个单词，按正常语速口播不超过 2 分 10 秒。
...
export function buildOpeningVariantPrompt(input) {
...
${task}
前三句按指定策略重做，不能只改几个词；中后段只换说法，不要整段重写成新故事。
卡片里的例句只示范句式，禁止复用例句中的戒指、婚礼、mafia 父亲等剧情。
