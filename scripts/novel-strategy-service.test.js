...
test("novel strategy draft, activation and rollback are versioned", () => {
  ...
  service.updateDraft({ diagnosis: { earlyDropPoints: 25 }, rewrite: { maxVariants: 3 }, audio: { voiceId: "voice-default" } });
  assert.equal(service.getState().draft.audio.voiceId, "voice-default");
