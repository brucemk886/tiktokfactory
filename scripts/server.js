...
import { attachPublishOutcome, chunkList, computeGroupReport, connectionIdsFromArchiveRows, mergePublishStats, parseShanghaiDate, resolveReportWindow } from "./official-group-report.js";
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "GET" && url.pathname === "/api/official-tiktok/ops-report") {
      ...
      const signals = officialAnalyticsArchive.getOperationSignals({
        days: reportArchiveDays(period, fromKey, toKey),
        videosPerAccount: 80,
      });
    ...
    if (req.method === "PATCH" && /^\/api\/novel-content\/novels\/[^/]+\/scripts\/[^/]+$/.test(url.pathname)) {
      if (!isLoopbackRequest(req)) return sendJson(res, 403, { error: "小说内容库仅允许在本机访问。" });
      try {
        const novelId = decodeURIComponent(url.pathname.split("/")[4]);
        const scriptId = decodeURIComponent(url.pathname.split("/")[6]);
        const script = novelContentLibrary.updateScript(novelId, scriptId, await readJsonBody(req));
        return sendJson(res, 200, { script, novel: novelContentLibrary.getNovel(novelId) });
      } catch (error) {
        return sendJson(res, Number(error.statusCode) || 400, { error: error.message || "保存文案失败。" });
      }
    }
    ...
    if (req.method === "GET" && url.pathname === "/api/asset-usage") {
      ...
      return sendJson(res, 200, getAssetUsageDashboard(root, String(url.searchParams.get("groupId") || ""), {
        ...
        publishRecords: readPublishRecords()
      }));
    }
...
function reportArchiveDays(period, fromKey, toKey) {
  const window = resolveReportWindow({ period, fromKey, toKey });
  return Math.max(2, Math.round((window.endAt - window.startAt) / 86_400_000));
}
