...
test("worker runs a render lane and a publish lane with configurable concurrency", () => {
  ...
  assert.deepEqual(lanes.map((lane) => [lane.name, lane.concurrency]), [["render", 1], ["publish", 1]]);
  ...
  assert.deepEqual(bad.map((lane) => lane.concurrency), [1, 1]);
  const settings = loadSettings(fs.mkdtempSync(path.join(os.tmpdir(), "factory-lanes-")));
  assert.equal(settings.renderConcurrency, 1);
