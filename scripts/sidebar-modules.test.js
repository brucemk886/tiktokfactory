...
test("business lines keep official and GeeLark navigation apart", () => {
  ...
  assert.deepEqual(
    ...
    ["mid-video", "schulte", "psychology-narrative", "quiz", "podcast", "ai", "mid-video-effects", "mid-video-ops-report", "mid-video-publish"]
  );
  assert.deepEqual(
...
test("retired local pages redirect to the online factory", () => {
  ...
  assert.equal(shouldRedirectLocalPageToFactory("/psychology-publish-sources"), true);
});
...
test("local reddit mix can refresh audio folders and push them to the factory", () => {
  ...
  assert.match(html, /id="assetFolderField"/);
  assert.match(html, /class="asset-folder-picker"/);
