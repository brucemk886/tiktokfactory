...
export function buildContentPerformance({items=[],records=[],accounts=[],videosByAccount=new Map(),now=Date.now(),media='photo'}){
 ...
 for(const i of items){const a=accountMap.get(i.connection_id),config=parseObject(i.config_json);if(!a||config.mediaType!==media)continue;
  ...
  rows.push({id:i.id,media,account:a.schema,accountName:a.profile?.username||a.username||a.label,group:a.groupName||'',source:i.source_key||i.source_id,variant:i.variant_id||'',style:i.style_id||'unknown',copyHash:i.copy_hash||'',title:copy.title||i.title||'未命名',copy,
