(() => {
  ...
  async function api(query='', options={}) {
    const response=await fetch('/api/psychology-peer-hits/production'+query,{cache:'no-store',...options});
  ...
  $('#boardDetail').addEventListener('click',event=>{
    const del=event.target.closest('[data-delete-job]');
    if(del)removeJob(del.dataset.deleteJob);
  });
