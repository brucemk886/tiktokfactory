...
function render() { const account = (state.data?.accounts || []).find((item) => item.schema === state.account); ui.$("#pageTitle").textContent = `${account?.label || state.account} 的全部视频`; ui.$("#backAccount").href = ui.withModule(`/official-account-detail?account=${encodeURIComponent(state.account)}`); renderVideos(); }
