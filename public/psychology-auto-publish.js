...
function itemState(item){
 ...
 if(item.publishOutcome==='published')return 'published';
 if(item.publishOutcome==='failed')return 'publish_failed';
 return ({queued:'queued',running:'producing',handoff:'producing',done:'producing',ready:'publishing',submitted:'scheduled',failed:'production_failed',cancelled:'cancelled'})[item.status]||'missing';
}
function batchStatus(items,groups=[]){
 ...
 if(states.every(s=>s==='published'||s==='cancelled'))return 'done';
 if(states.includes('published'))return 'partial';
 if(groups.some(g=>g.retrying)||states.some(s=>['producing','publishing'].includes(s)))return 'running';
 if(states.some(s=>['production_failed','publish_failed'].includes(s)))return 'failed';
 if(states.includes('queued'))return 'queued';
 if(states.includes('scheduled'))return 'scheduled';
 return 'unknown';
}
function itemLabel(status){return {published:'发布成功',production_failed:'制作失败',publish_failed:'发布失败',queued:'排队中',producing:'制作中',scheduled:'待发布',publishing:'发布中',missing:'记录缺失',cancelled:'已取消'}[status]||'记录缺失';}
function publicationCounts(items){
 const counts={published:0,production_failed:0,publish_failed:0,queued:0,producing:0,scheduled:0,publishing:0,missing:0,cancelled:0};
...
function renderBatches() {
  ...
  $('#queuedCount').textContent=tones.filter(s=>s==='queued'||s==='scheduled').length;
  $('#runningCount').textContent=tones.filter(s=>s==='running').length;
  $('#attentionCount').textContent=tones.filter(s=>s==='failed'||s==='unknown'||s==='partial').length;
...
function renderBatchDetail(b){
 const labels={queued:'等待执行',running:'执行中',done:'生成完成',submitted:'已提交中台',failed:'失败',cancelled:'已取消',missing:'记录缺失',handoff:'等待卡片渲染'};
